...
export function AIChatDialog({
  ...
  > => {
    const res = await axios.get<
      BaseResponse<GetAllSessionResponse[]>
    >(`${AppConfig.baseURL}/chatbot/sessions`);
  ...
  const sessionClickHandler = async (
    sessionId: string,
  ) => {
    setActiveSessionId(sessionId);

    const res = await axios.get<
      BaseResponse<GetChatHistoryResponse[]>
    >(
      `${AppConfig.baseURL}/chatbot/chat-history?chat_session_id=${sessionId}`,
    );
  ...
  const createNewSession = async () => {
    const res = await axios.post<
      BaseResponse<CreateSessionResponse>
    >(
      `${AppConfig.baseURL}/chatbot/create-session`,
    );
  ...
  const deleteSession = async (
    ...
    await axios.delete(
      `${AppConfig.baseURL}/chatbot/delete-session`,
      {
        data,
      },
    );
  ...
  const handleSend = async () => {
    ...
    >(
      `${AppConfig.baseURL}/chatbot/send-chat`,
      request,
    );
          ...
          return {
            ...
            messages: [
              ...s.messages.slice(0, -1),
              {
                ...
                role:
                  res.data.data.send.chat ===
                  "model"
                    ? "assistant"
                    : "user",
                timestamp: new Date(
                  res.data.data.send.created_at,
                ),
              },
              {
                ...
                role:
                  res.data.data.reply.chat ===
                  "model"
                    ? "assistant"
                    : "user",
                timestamp: new Date(
                  res.data.data.reply.created_at,
                ),
              },
            ],
          };
